...
dynatop <- R6Class(
    ...
    public = list(
       ...
       plot_state = function(state=c("s_sf","s_rz","s_uz","s_sz","s_snow","s_liquid")){ #,add_channel=TRUE){ #add snow
    ...
    private = list(
        ...
        info = list(sf = setNames(as.integer(1:3),c("cnst","kin","comp")),
                    ...
                    snow = setNames(as.integer(1),c("hbv")),
                    output = setNames(1:16, c("precip","pet","aet",
                                              "q_sf","q_sf_in","q_sz","q_sz_in",
                                              "s_sf","s_rz","s_uz","s_sz",
                                              "v_sf_rz","v_rz_uz","v_uz_sz",
                                              "s_snow","s_liquid")) #add snow
                    ),
        digest_hru = function(h, use_states, delta){ ## check HRU returns a text string of errors
            ...
            ## check states
            snm <- c("s_sf","s_rz","s_uz","s_sz","s_snow","s_liquid")
            ...
            ## convert for C++
            for(ii in c("sf","rz","uz","sz", "snow")){ ## convert type to integer
                h[[ii]]$type <- private$info[[ii]][ h[[ii]]$type ]
            }

            if( !use_states ){ 
              h$states[c("s_sf","s_rz","s_uz","s_sz")] <- NA 
              h$states[c("s_snow","s_liquid")] <- 0.0 
            }
            return(h)
