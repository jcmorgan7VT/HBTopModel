...
void dt_init(Rcpp::List mdl, // hru data frame
  ...
  for(int ii=0; ii<nhru; ++ii){
    ...
    svec["s_snow"] = hrus[ii].s_snow / hrus[ii].area;
    svec["s_liquid"] = hrus[ii].s_liquid / hrus[ii].area;
  };
...
void dt_sim(Rcpp::List mdl, // list of HRUs
  ...
  for(int tt = 0; tt < obs_matrix.nrow(); ++tt) {
    ...
    for(int ii=0; ii<nhru; ++ii){
      if( hrus[ii].area > 0.0){
	mbv[0] += (hrus[ii].s_sf + hrus[ii].s_rz + hrus[ii].s_uz - hrus[ii].s_sz + hrus[ii].s_snow + hrus[ii].s_liquid); // initial state volume
    ...
    for(int ii=0; ii<nhru; ++ii){
      if( hrus[ii].area > 0.0){
	mbv[4] += (hrus[ii].s_sf + hrus[ii].s_rz + hrus[ii].s_uz - hrus[ii].s_sz + hrus[ii].s_snow + hrus[ii].s_liquid); // final state volume
      }
    }
