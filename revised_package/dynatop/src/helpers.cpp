...
std::vector<hru> makeHRUs(Rcpp::List mdl){
  ...
  for(int ii=0; ii<nhru; ++ii){
    ...
    Rcpp::List snow_list = m["snow"]; // add snow
    Rcpp::List pcp_list = m["precip"];
    Rcpp::List pet_list = m["pet"];
    Rcpp::List temp_list = m["temp"]; // added temp
    ...
    hrus.push_back( hru( id, //m["id"], // id passed explicitly
			 ...
			 Rcpp::as<std::vector<double>>(snow_list["parameters"]), // Add this
			 ...
			 Rcpp::as<std::vector<int>>(temp_list["idx"]), Rcpp::as<std::vector<double>>(temp_list["fraction"]), // Add this
...
Rcpp::List makeStateList(std::vector<hru> &hrus){
  ...
  for(int ii=0; ii<nhru; ++ii){
    Rcpp::NumericVector s = Rcpp::NumericVector::create(
							...
							Rcpp::Named("s_snow", hrus[ii].s_snow / hrus[ii].area), // Add this
							Rcpp::Named("s_liquid", hrus[ii].s_liquid / hrus[ii].area)); // Add this
