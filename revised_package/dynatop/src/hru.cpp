...
hru::hru(int const id_,
	 ...
	 std::vector<double> const snow_param_, // Add snow param
	 ...
	 std::vector<int> const temp_lnk_id_, std::vector<double> const temp_lnk_frc_, // Add temp
  ...
  t_d(uz_param_[0]), sz_param(sz_param_), snow_param(snow_param_), // snow param init
  precip_lnk_id(precip_lnk_id_), precip_lnk_frc(precip_lnk_frc_),
  pet_lnk_id(pet_lnk_id_), pet_lnk_frc(pet_lnk_frc_),
  temp_lnk_id(temp_lnk_id_), temp_lnk_frc(temp_lnk_frc_), // Initialize temp link
  ...
  s_snow(states_[6]), s_liquid(states_[7]) // Add this. Note: states_[4] and [5] are usually q_sf/q_sz in the R vector
{
  ...
  s_uz *= area;
  s_sz *= area;
  s_snow *= area;   // Add this
  s_liquid *= area; // Add this
...
void hru::update_met(std::vector<double> &obs){
  ...
  temp = 0.0;
  for(long unsigned int ii=0; ii<temp_lnk_id.size(); ++ii){
    const int &i = temp_lnk_id[ii];
    const double &f = temp_lnk_frc[ii];
    temp += f * obs[i]; // no area
  }
}
	       ...
	       double const &vtol, double const &etol, int const &max_it, double const &Dt)
{
  ...
  // ==========================================
  // ADD SNOW ROUTINE HERE
  // ==========================================
  double T_T = snow_param[0];  // Threshold Temperature
  double C_f = snow_param[1];  // Degree-Day Melt Factor
  double C_wh = snow_param[2]; // Water Holding Capacity of Snowpack
  double raw_precip = precip; // Save original precip for partitioning
  
  // STRICT BYPASS: Warm weather AND empty snowpack
  if (temp >= T_T && s_snow < 1e-8) {
    
    // Pass raw precip straight through without touching Dt or s_liquid
    s_snow = 0.0;
    s_liquid = 0.0;
    ...
    // Add to states (Depth only, no area)
    s_snow += Dt * p_snow;
    s_liquid += Dt * p_rain;
    
    // 2. Melting (REMOVED 'area' to keep it as a depth calculation)
    if(temp >= T_T && s_snow > 0.0){
      double melt = std::min(s_snow, C_f * (temp - T_T) * Dt); 
      s_snow -= melt;
      s_liquid += melt;
    }
    
    // 3. Yield to surface
    double max_liquid = C_wh * s_snow;
    double effective_precip = 0.0;
    
    if (s_liquid > max_liquid) {
      effective_precip = (s_liquid - max_liquid) / Dt; // Convert back to rate
      s_liquid = max_liquid;
    }
    
    // Replace precip with the snowpack yield
    precip = effective_precip; 
  }
  // ==========================================
